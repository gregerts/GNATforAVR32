...
package System.BB.Protection is
   ...
   procedure Leave_Kernel_No_Change;
   pragma Inline (Leave_Kernel_No_Change);
   --  Leave_Kernel_No_Change may be called when the access to kernel
   --  data finishes instead of Leave_Kernel if the code is known not
   --  to alter the thread queue or thread priority. Interrupts are
   --  enable to the appropriate level (according to the active
   --  priority of the running thread).
